	...
	$sql = "SELECT DISTINCT 
				   ...
				   weight, 
				   gross_amount, 
				   BOL FROM dayton as thedata
			INNER JOIN (SELECT customer_id,carrier_id FROM carriers_customers) as cc
			ON cc.customer_id = '".$customer."' AND cc.carrier_id = '".$carrier."'
			WHERE thedata.customer_id = '".$customer."' AND thedata.carrier_id = '".$carrier."'
			AND thedata.statement_number = '".$invoice."'";
		 $query = mysql_query($sql,$con);
	if (mysql_num_rows($query) <> 0)
	{
	?>
	<center><h5>All Shipments: <?echo $foundCustomerName['customer_name'].", ".$foundCarrierName['carrier_name'].", ".$invoice;?></h5></center>
