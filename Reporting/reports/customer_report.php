<?
	    ...
	    $sql="SELECT line_item_id,
			  ...
			  LEFT JOIN 
			  (SELECT carrier_name, carrier_id FROM carriers) 
...
?>
		<center><h5>All Shipments: <?echo $foundCustomerName['customer_name'];?></h5></center>
		...
		<?
			...
			<tr>
				<td style="border:1px solid; border-color:#A4A4A4; border-radius:5px; text-align:center;width:100px;font-size:10px;">
					<a style="display:block;" href="?form=line_item_information&report=customer_report&line_item=<?echo$row['line_item_id']?>&customer=<?echo $customer?>" title="view">
