	...
	<div id="tabs-1">
		...
		if(isset($_REQUEST['carrier']) && $_REQUEST['carrier'] != "NULL"){
		    ...
		    if ($customer == "NULL"){
				...
				$sqlRvw = "";
				$sqlRvw = " INNER JOIN min_max as mm
		...
		if(isset($_REQUEST['invoice']) && $_REQUEST['invoice'] != "NULL" && strlen($_REQUEST['invoice']) > 0){
			...
			if ($sqlRvw == ""){
				...
				$sqlRvw .=  " AND thedata.statement_number = '".$invoice."'
							  ORDER BY inv_id";
			}  
		}
		...
		&& isset($_REQUEST['to']) && $_REQUEST['to'] != "--End Date--" && strlen($_REQUEST['to']) > 0){
			...
			if ($sqlAprvd == ""){
				...
				$sqlRvw .= " AND thedata.invoice_date BETWEEN '".$from."' AND '" .$to."'
							 ORDER BY inv_id";
			} 
		}
	if ($sqlRvw != ""){
		$sqlRvwFinal = $sqlStrt ." ". $sqlRvw;
		display_table($sqlRvwFinal,$sqlGetMinMax,$customer,$carrier,$invoice,$to,$from,1);
	    //echo $sqlGetMinMax;
	}
	?>
	...
	<div id="tabs-2">
		...
		if(isset($_REQUEST['invoice']) && $_REQUEST['invoice'] != "NULL" && strlen($_REQUEST['invoice']) > 0){
				...
				if ($sqlDspt == ""){
					...
					$sqlDspt .=  " AND thedata.statement_number = '".$invoice."'
								   ORDER BY inv_id";
		...
		&& isset($_REQUEST['to']) && $_REQUEST['to'] != "--End Date--" && strlen($_REQUEST['to']) > 0){
				...
				$sqlGetMinMax = "SELECT * FROM min_max as mm
					...
					$sqlDspt .= " AND thedata.invoice_date BETWEEN '".$from."' AND '" .$to."'
								 ORDER BY inv_id";
				} 
		}
	if($sqlDspt != ""){
		$sqlDsptFinal = $sqlStrt ." ". $sqlDspt;
		display_table($sqlDsptFinal,$sqlGetMinMax,$customer,$carrier,$invoice,$to,$from,2);
	    //echo $sqlDsptFinal;
	}
	?>
	</div>
	<div id="tabs-3">
		...
		&& isset($_REQUEST['to']) && $_REQUEST['to'] != "--End Date--" && strlen($_REQUEST['to']) > 0){
			...
			if ($sqlAprvd == ""){
				...
				$sqlAprvd .= " AND thedata.invoice_date 
				               BETWEEN '".$from."' AND '" .$to."'
							   ORDER BY inv_id";
			} 
		}
	...
	<div id="tabs-4">
		...
		&& isset($_REQUEST['to']) && $_REQUEST['to'] != "--End Date--" && strlen($_REQUEST['to']) > 0){
			...
			if ($sqlInv == ""){
				...
				$sqlInv .= " AND thedata.invoice_date 
				             BETWEEN '".$from."' AND '".$to."'
							 ORDER BY inv_id";
			} 
		}
